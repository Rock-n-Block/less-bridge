...
function ModalManualSwapContent(props) {
  ...
  const handleClose = () =>
    dispatch(modalActions.toggleModal({ isOpen: false, text: null }));
  ...
  return (
    <div className="modal-manual-swap">
      <div className="modal-manual-swap-header">Manually</div>
      ...
      <div className="modal-manual-swap-comment">
        ...
        <div>
          address in {networkTo.split("-").join(" ")} please make the following
          transaction
        </div>
      </div>

      <div>
        <div className="modal-manual-swap-label">Specify Asset</div>
        <div className="modal-manual-swap-field">
          <div className="modal-manual-swap-field-content">
            {copied === "asset" ? "Copied!" : asset}
          </div>
          <CopyToClipboard text={assetCopy} onCopy={() => handleCopy("asset")}>
            <div className="modal-manual-swap-button-copy-container">
              <IconCopy className="modal-manual-swap-button-copy" />
            </div>
          </CopyToClipboard>
        </div>

        <div className="modal-manual-swap-label">Address</div>
        <div className="modal-manual-swap-field">
          <div className="modal-manual-swap-field-content">
        ...
        <div className="modal-manual-swap-label">Amount to send</div>
        <div className="modal-manual-swap-field">
          <div className="modal-manual-swap-field-content">
            {copied === "amount" ? "Copied!" : amount}
          </div>
          <CopyToClipboard text={amount} onCopy={() => handleCopy("amount")}>
            <div className="modal-manual-swap-button-copy-container">
              <IconCopy className="modal-manual-swap-button-copy" />
            </div>
          </CopyToClipboard>
        ...
        <div className="modal-manual-swap-label">
          ...
          <span className="color-red">
            (IMPORTANT! MEMO FIELD IS MANDATORY)
          </span>
        </div>
        <div className="modal-manual-swap-field">
          <div className="modal-manual-swap-field-content">
          ...
          <CopyToClipboard text={memo} onCopy={() => handleCopy("memo")}>
            <div className="modal-manual-swap-button-copy-container">
              <IconCopy className="modal-manual-swap-button-copy" />
            </div>
          </CopyToClipboard>
      ...
      <div className="modal-manual-swap-button-container">
        <div className="button mt20" onClick={handleClose}>
          Close
        </div>
      </div>
    </div>
